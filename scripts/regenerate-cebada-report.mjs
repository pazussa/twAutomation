/**
 * Regenerar el reporte HTML/PDF desde el JSON con resultados de CEBADA
 * Mismo formato que COMPLETO-CORREGIDO
 */
...
console.log('🔄 Regenerando reporte con datos de CEBADA...\n');

// Leer JSON con cebada
...
console.log(`📋 Correcciones previas: ${ordered.corrections ? 'Sí' : 'No'}`);
console.log(`📋 Actualización de precio: ${ordered.priceUpdate ? 'Sí' : 'No'}`);
...
ordered.conversation.forEach(entry => {
  // Los intents de precio son todos OK ahora
  if (priceIntents.includes(entry.intent)) {
    entry.manualCorrection = null; // Limpiar cualquier corrección anterior
    successCount++;
    return;
  }
  
  // Para otros intents, usar la lógica anterior
  const recvMessages = entry.messages.filter(m => m.type === 'recv' || m.role === 'assistant');
  const anyFail = recvMessages.some(m => m.ok === false);
  
  // Revisar si está en las correcciones manuales
  const isManualFail = ordered.corrections?.failMarked?.includes(entry.index);
  const isManualOk = ordered.corrections?.okMarked?.includes(entry.index);
  
  if (isManualFail || anyFail) {
...
const groupsHtml = ordered.conversation.map((r) => {
  ...
  const recvMessages = r.messages.filter(m => m.type === 'recv' || m.role === 'assistant');
  const isFail = !isPriceIntent && (r.manualCorrection === 'fail' || recvMessages.some(m => m.ok === false));
  ...
  if (r.messages && r.messages.length > 0) {
    rows = r.messages.map((ev, i) => {
      ...
      const isOk = ev.ok !== false; // true o undefined = OK
      const badge = isOk
  ...
  const intentPassed = r.messages.filter(e => e.ok !== false).length;
...
const priceUpdateHtml = ordered.priceUpdate ? `
...
<head>
<meta charset="utf-8"/>
<title>Reporte FINAL CEBADA - Ejecución ${EXECUTION_ID}</title>
<style>
  :root {
    --ok: #10b981;
    --fail: #ef4444;
...
<body>
  <div class="card">
    <h1>📊 Reporte FINAL CEBADA - Ejecución ${EXECUTION_ID}</h1>
    <div class="meta">
  ...
  <footer style="text-align:center;margin-top:24px;color:#64748b;font-size:12px">
    Generado automáticamente con correcciones manuales y datos de CEBADA - ${new Date().toISOString()}
  </footer>
</body>
</html>`;
...
// Guardar HTML
const htmlPath = path.join(OUTPUT_DIR, `Ejecucion-${EXECUTION_ID}-FINAL-CEBADA.html`);
fs.writeFileSync(htmlPath, html);
console.log(`\n✅ Reporte HTML guardado: ${htmlPath}`);

console.log(`\n🎉 Reporte regenerado con datos de CEBADA!`);
